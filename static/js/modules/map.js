...
define(['legend', 'cartodb'], function(legend) {
	...
	var cartoLayer = cartodb.createLayer(map, {
	  ...
	  sublayers: [{
		...
		sql: "SELECT e.estacio, e.cartodb_id,  e.the_geom_webmercator, b.ecostrimed, b.data FROM estacions e INNER JOIN indexbio b ON e.estacio=b.estacio WHERE b.data < '2014-07-01'::date AND b.data > '2014-01-01'",
	...
	.done(function(layer) {
	     layer.setZIndex(7);
		 legend.createSwitcher(map, function(){});
	     // info window
	     // if we need a different template: http://requirejs.org/docs/download.html#text
	     /*var sublayer = layer.getSubLayer(0);
	     sublayer.infowindow.set('template', $('#infowindow_template').html());*/
	     cdb.vis.Vis.addInfowindow(map, layer.getSubLayer(0), ['ecostrimed', 'data', 'cartodb_id']);
